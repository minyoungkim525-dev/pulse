export function detectPatterns(entries) {
  ...
  // Recovery moment
  const firstEntry = entries[entries.length - 1]; // oldest entry
  const latestEntry = entries[0]; // newest entry

  if (latestEntry.mood > firstEntry.mood + 2) {
    insights.push({
      type: 'positive',
      icon: '💪',
      title: 'Recovery Proof',
      message: `You bounced back from ${firstEntry.mood}/10 to ${latestEntry.mood}/10`
    });
  }
